...
【絶対的禁止事項】
- **単なる箇条番号の暗記を問う問題（例：「〇〇について規定しているのは箇条いくつか？」といった問題）は絶対に作成しないでください。** これらは実務的な学習価値がありません。
- 規格の構成や番号そのものではなく、規格の「内容」「意図」「実務場面での的確な適用方法」を問う問題に絞ってください。箇条番号はあくまで「解説の根拠となる参照情報」としてのみ記述してください。

【重要】毎回同じような出題にならないよう、以下の【出題パターン】からランダムに1つを選んで問題文を構成してください。

【出題パターン】
パターンA（実務シナリオ型）:
架空の機械設計や現場のトラブル場面を提示し、提供された規格データに照らし合わせてどの対応が正しいか（または誤っているか）を問う。
