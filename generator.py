...
1. 箇条番号・参照先の捏造禁止: 提供されたテキスト内に存在しない箇条番号（例：存在しない6.3.1など）や項番を絶対に作り出さないこと。また、索引や別の規格への参照先についても、テキスト通りに正確に引用し、勝手な解釈を加えないこと。
2. 規格体系・優先順位の厳守: 「3ステップメソッド（1:本質的安全設計方策, 2:安全防護及び/付加保護方策, 3:使用上の情報）」などの階層構造や優先順位を説明する際は、事前知識で勝手に言葉を補ったり順番を解釈せず、テキストの定義をそのまま正確に使用すること。

【重要】毎回同じような「～について最も適切なものを選べ」という問題にならないよう、以下の【出題パターン】からランダムに1つを選んで問題文を構成してください。

【出題パターン】
パターンA（実務シナリオ型）:
架空の機械設計や現場のトラブル場面を提示し、提供された規格データに照らし合わせてどの対応が正しいかを問う。

パターンB（誤り指摘型）:
4つの選択肢の中に、実務者が陥りやすい「もっともらしいが規格上は間違っている」設計方針や解釈を混ぜ、正しいもの（または誤っているもの）を指摘させる。

パターンC（理由・根拠型）:
「なぜその措置・数値が必要なのか」という規格の背景にある考え方や理由を提供データに基づいて問う。
...
- 誤選択肢には、「昔の規格なら正解だった考え方」や「安全側への倒しすぎで非現実的なもの」など、プロでも迷う巧妙なものを混ぜてください。ただし解説では、それが誤りである根拠を提供データから明確に示してください。
...
[
  {{
    ...
    "answer": "A: 選択肢1",
    "explanation": "解説（解説は提供されたテキストの記述と正確な箇条番号を根拠にすること）"
  }},
  ...
]
"""
